...
exports.removeEvent = function (req, res, next) {
  ...
  userEvents.find({ userEmail: email }).then(function (response) {

    for (var i = 0; i < response.length; i++) {
      console.log(response[i].events[0]);

      if (response[i].events[0]._id == event._id) {
        userEvents.remove({ _id: response[i]._id }).then(function (response) {
...
exports.editUser = function (req, res, next) {
  ...
  User.updateOne({ email: oldEmail }, { $set: { firstName: firstName, lastName: lastName, email: newEmail, phoneNumber: phoneNumber } }).then(response => console.log(response)).catch(err => console.log(err));
};